...
- `loadOnDeviceModel` — Try to download / install the on-device locale model during prewarm. Default: `true`.

## Load on-device model {#load-on-device-model}
...
On Android, prewarm currently always attempts model prepare when `onDevice` is set (the shared option still exists; default is `true`).
...
- Download the assets (if needed) — gated by `loadOnDeviceModel` on iOS 26+ Speech/Dictation
