...
Covers `ErrorDictionary` completeness vs 4.10 codes, `PermissionStatus`, volume/active helpers, `RecognizerRef` method mapping, and a render smoke test of the listen UI.
...
Covers AutoStopper/timers, repeating filter, transcriber param hash, permission mapping, engine candidate order, RMS/volume math, synthetic PCM AudioLevelTracker.
...
Requires Android SDK (`ANDROID_HOME` or `local.properties`). Covers AutoStopper, filters, volume/RMS, permission mapping, locale tags, on-device prepare decisions, result batch helpers, and the emulator silence → `RecognitionTaskFailed` mapping.
